...
# Run experiments (control vs treatment comparison)
python run_experiment.py                    # Generates 12 scenarios in both control and treatment modes
```
...
Alternative models: `openai/gpt-4`, `anthropic/claude-3-opus`, `meta-llama/llama-3.1-70b-instruct`, `google/gemini-pro`

- **`create_scenario.py`**: Entry point for generating a single scenario - edit the `ScenarioConfig` here

- **`run_experiment.py`**: Batch experiment runner that generates 12 historical scenarios using both control (`generator.py`) and treatment (`reachability_research/generator_reachability.py`) generators. Outputs to `output/control/` and `output/treatment/` with raw code saved to `code/` subdirectories.

- **`reachability_research/`**: Contains experimental generator variant (`generator_reachability.py`) with modified prompts for research comparison

### Experiment Output Structure

`run_experiment.py` generates 12 historical scenarios in both modes:
```
output/
├── control/              # Standard generator.py
│   ├── code/             # Generated Python files
│   └── c_*.aoe2scenario  # Compiled scenarios
└── treatment/            # generator_reachability.py variant
    ├── code/
    └── t_*.aoe2scenario
```
